...
- Produces: `drafts_sweep` and `drafts_follow_up` skip users who are locked or out of draft quota.
...
In `_due_users`, filter the loop so an ineligible user is never even loaded for drafting — running out of quota must not raise to the user, it just quietly produces no draft:

```python
    for row in rows:
        if not await may_draft(db, row.user_id, datetime.now(timezone.utc)):
...
After each successful draft creation in the sweep loop, record usage. In `drafts_sweep`, replace the `created += sweep.sweep_user(user_id, config)` line with:

```python
            made = sweep.sweep_user(user_id, config)
            created += made
            if made:
                run_async(
                    with_worker_session(
